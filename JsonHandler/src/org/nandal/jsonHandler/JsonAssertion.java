...
public class JsonAssertion extends AbstractScopedAssertion implements Serializable, Assertion {
    ...
    private static final int VALIDATE_JSON_SCHEMA = 1 << 1;
    private static final int VALIDATE_XML_SCHEMA = 1 << 2;
    private static final int CONTAINS = 1 << 3;
    private static final int NOT = 1 << 4;
    private static final int EQUALS = 1 << 5;
    private static final int SUBSTRING = 1 << 6;
    private static final int OR = 1 << 7;

    // Mask should contain all types (but not NOT nor OR)
    private static final int TYPE_MASK = VALIDATE_JSON_SCHEMA | VALIDATE_XML_SCHEMA | CONTAINS | EQUALS | MATCH | SUBSTRING;
    ...
    public boolean isValidateXmlSchemaType() {
        return (getTestType() & VALIDATE_XML_SCHEMA) != 0;
    }


    public boolean isValidateJsonSchemaType() {
        return (getTestType() & VALIDATE_JSON_SCHEMA) != 0;
    }
    ...
    public void setToValidateXmlSchemaType() {
        setTestTypeMasked(VALIDATE_XML_SCHEMA);
    }
    public void setToValidateJsonSchemaType() {
        setTestTypeMasked(VALIDATE_JSON_SCHEMA);
    }
    ...
    private AssertionResult evaluateResponse(SampleResult response) {
        ...
        boolean validateJsonSchema = isValidateJsonSchemaType(); // do it once outside loop
        boolean validateXmlSchema = isValidateXmlSchemaType(); // do it once outside loop
        ...
        try {
            ...
            for (JMeterProperty jMeterProperty : getTestStrings()) {
                ...
                if (validateXmlSchema) {
                    found = XmlUtil.validateSchema(toCheck, pattern);
                } else if (validateJsonSchema) {
                    found = JsonUtil.validateSchema(toCheck, pattern);
                } else if (contains) {
    ...
    private String getFailText(String stringPattern, String toCheck) {
        ...
        case VALIDATE_XML_SCHEMA:
        	sb.append(" expected to have XML Schema ");
        	break;
        case VALIDATE_JSON_SCHEMA:
        	sb.append(" expected to have JSON Schema ");
        	break;
        case CONTAINS:
        case SUBSTRING:
